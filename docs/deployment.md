...
workflow runs `ruff`, `black` and `pytest`, the suite under coverage with a
floor on the statements and one on the branches. Beside them it builds the
image and smoke tests it the way `buildpush.sh` does, with its layers kept in
the Actions cache so the collection is prepared again only when its branch
moves. Once both have passed it builds again from that cache and pushes three
tags: the commit's short sha, `latest`, and the version, that last one only
when it is not published already. It needs two repository secrets, `DOCKERHUB_USERNAME` and
`DOCKERHUB_TOKEN`, the second an access token from Docker Hub rather than an
account password, scoped read, write and delete.

A pull request runs the tests alone. The publish job is where the Docker Hub
...
A machine carrying `deploy_metrics_proxy_enabled` serves all of them on 9464
instead, each on its own path (`/metrics/node`, `/metrics/ha`, ...), over TLS,
and serves none of them on the administration network: the role moves its
exporters to the loopback and puts one nginx in front. The fan out follows
that variable per machine, and verifies the certificate against the copy it
read over the SSH connection a run already makes. D64 and D65 record it, and
`metrics_proxy_ca_file` is how a site with a PKI replaces the pinning.
...
# answering on it.
podman pull docker.io/insatomcat/seapath-webui:0.3.185
