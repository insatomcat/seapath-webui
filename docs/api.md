...
| `/usage` | What each machine **consumes**, and which guest or container consumes it: CPU, memory, disks and network, over the last five minutes this browser read. Read only |
...
on it, as counters. See [D67](decisions.md#d67).

| Method | Path | Description |
|---|---|---|
| GET | `/usage` | One row per machine with an address, in the inventory's order. `node` is what its `node_exporter` publishes: `read_at`, the machine's own clock at the scrape; `housekeeping` and `isolated`, each `cpus`, `total_seconds` and `idle_seconds` over every CPU of that side, `iowait` counted as idle; `load1`, `load5`, `load15`; `memory` in bytes, the hugepage pool included; `filesystems` with a size and what is free, tmpfs, overlay and the kernel's own left out; `disks`, the read and written bytes and the time with I/O in flight, logical volumes left out since their I/O is counted on the disk under them; `interfaces`, received and sent bytes, the link `speed_bytes` and a `kind` of `physical`, `logical`, `workload` or `loopback`. `guests` from `libvirt-exporter`, each with `cpu_seconds`, `memory_bytes`, the disk and network bytes and its `interfaces`; `containers` from `prometheus-podman-exporter`, the same, named by the container's name. `guests_read_at` and `containers_read_at` are when each answer arrived here, neither exporter publishing a clock. `node_reach`, `guests_reach` and `containers_reach` say whether each answered and why not. `viewer` |

**Counters, and the moment each was read.** Every figure is as the exporter published it, in
seconds or bytes since some start. A rate is the difference between two
readings over the time between them, taken on the node's own clock for the
machine and on this service's for the workloads, and the caller keeps the
first reading: this service keeps nothing. A counter lower than in the
reading before was reset, by a reboot or a guest started again, and that
interval gives no rate. A counter the exporter did not publish is `null`, and
so is one for a guest or container that is not running.

**Every reading reaches the machines.** The scrape window the other pages
share is not in front of this one, and the endpoint takes no `fresh`: an
answer kept for a few seconds and divided by the time since it was first read
is a machine that did nothing. It does not empty the window either, so the
pages that share one scrape keep sharing it while this one is read every five
seconds.

**A machine is its three exporters, each on its own.** One that answers
nothing on `node_exporter` is a row with the reason and no counters. One
outside the `hypervisors` group runs no `libvirt-exporter` and no
