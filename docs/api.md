...
| `/backup` | Where the guests are backed up to, what a full backup would weigh, what the backup server holds, and the four acts of the `backup_restore` role |
| `/updates` | What an upgrade would bring to each machine, as the last check found it, and the upgrade, one machine at a time |
| `/logs` | The journal of every machine at once: a scope, a window, and the lines of the whole cluster interleaved. Read only, stored nowhere |
| `/usage` | What each machine **consumes**, and which guest or container consumes it: CPU, memory, disks and network, over the last five minutes, which this service keeps while somebody uses it. Read only |
...
| Read the journal of the cluster | `viewer`, moved with `SEAPATH_WEBUI_LOGS_MIN_ROLE` |
| Cancel a run, check the machines for software updates | `operator` |
...
| GET | `/vms/displays` | Which guests have a VNC display: a cluster guest's from the domain XML Ceph holds, a standalone guest's from `virsh dumpxml` on its machine |
...
| PUT | `/realtime/pool/{host}/strategy` | Set one machine's `seapath_alloc_strategy` to `spreading`, `packing` or `repacking`, as one commit on its entry, `If-Match` on the commit hash. Then one run of `seapath_setup_seapath_alloc` narrowed to that machine templates `/etc/seapath/alloc.yaml`, which `seapath-alloc` reads at its next allocation. Answers with the commit and `run_id`, neither when the machine already receives this value, the role default `spreading` included. `404 unknown_host`, and `409 precondition_failed` before anything is committed when the playbook cannot run from here. Each node of `/realtime/pool` carries the value it receives as `alloc_strategy`, and `alloc_strategy_on` says where the inventory sets it: `host` or a group name. `admin` |
...
- `GET /vms/displays` answers `{"guests": {"<name>": true | false | null}}`,
  whether each guest's definition carries a VNC `<graphics>`. A cluster
  guest's is the `xml` metadata `vm_manager` keeps on its system image. A
  standalone guest's is the domain its machine runs, read with `virsh
  dumpxml` in one ssh per machine the libvirt exporter reports guests on.
  `null` is an image Ceph, or a machine, did not answer for. A cluster guest
  with no image, and a standalone guest nothing reports, is absent. The page
  offers the console on `true` alone.
...
| POST | `/vms/{name}/delete-sources` | Delete the files a created guest was made from: the image from the artefacts, and an XML from the versioned folder as a commit (`files: remove <path>`). The files are the ones the commit that forgot the guest's creation names in its `Source-File` trailers, still held by this node and named by no entry of the inventory. Answers with the `files` deleted and the `commits`. `409 no_sources` when nothing is left to delete. See [D50](decisions.md#d50). `admin` |
...
**`root_password` answers the console, and reaches no entry.** A guest whose network does not come up is reached at its console, and an account whose password is locked answers nothing there, which is the state a SEAPATH VM image leaves root in. Checked here and written nowhere: this is the one thing the form asks for that the inventory never carries, in any form. A hash committed to this repository is replicated to every machine the file declares, readable there by everybody the repository is, kept in the history for good, and an attacker holding it gets to try offline and without a rate limit.

So it is sent a second time, on the run that creates the guest, as `root_passwords` of `POST /runs`. That run splices a `users` member into the copy of the inventory it stages for itself, `root` with `lock_passwd: false` and the `$6$` string `/etc/shadow` holds, and wipes that copy when it ends. `lock_passwd: false` is required rather than tidy: cloud-init locks every account it touches unless told otherwise, and it does that after setting the password.

Which password root gets is the operator's call, on their own guest: the only refusals are a box checked over an empty field, and a character the file could not carry. No SSH login is opened, since the seed leaves `ssh_pwauth` as the image set it, and what reaches the guest over the network is the account above, by key. Absent, nothing is written and the guest's root account stays as its image built it.
...
The upstream `backup_restore` role installs four programs in `/usr/local/bin`
and a whiptail menu over them. `backup_full.sh` exports every guest's RBD
images as qcow2 and rsyncs them to a backup server, `backup_inc.sh` exports RBD
diffs against the latest snapshot into the same directory, `restore_vm.sh`
brings one guest back from a chosen date, and `backup_du.py` estimates what a
full backup would weigh. This is the port of the four; the menu is the half
that has no place here, because it reads and writes `/etc/backup-restore.conf`
and writing a file on a host is the one thing this service does not do. See
...
| Method | Path | Description |
|---|---|---|
| GET | `/backup` | `settings` is the seven variables as the cluster members resolve them, each with its label, the conf key it corresponds to, the variable it is written under and `on_machine`, which is what this node's own conf file holds for it; `target` is `<server>:<directory>`; `conf_path`, `conf_found` and `conf_only` say whether that file was read and whether it holds values the inventory does not; `warnings` names a setting written in more than one place, and a value where the file and the inventory differ; `runs_on` is the cluster member the backups run on and every reading here asks, the first hypervisor of the cluster by name. Read off the disk, so it answers at once |
| GET | `/backup/catalogue` | What the backup server holds, asked now: one SSH connection to the member the backups run on, `sudo` there, and the directory listed. `read_from` names the member and `read_at` says when. A read, so it takes no run and no lock. See [D54](decisions.md#d54) |
| GET | `/backup/estimate` | `rbd du` summed per guest with the two filters applied, carrying `included` and `excluded`. Its own endpoint because it is its own cost: with no fast-diff map `rbd du` walks every object of an image, so it is never on the path of the page being drawn. It runs over SSH on the member the backups run on, `rbd ls` first and then `rbd du` of each image the filters select, so no other image is walked. A Ceph that does not answer in time is reported in `error` rather than failing the request. `used_bytes` is a ceiling: the rows `rbd du` answers are deltas between snapshots, so an image is worth its rows added up, bounded by what it provisions |
| GET | `/backup/staging` | The two staging directories on the member the backups run on, over one SSH connection: whether each exists, the file system that holds it or would hold it, its size and what is free. `host` names the member. A full backup writes a qcow2 of every selected guest there before it sends anything, and on a machine installed from the ISO that file system is a root of a few tens of gigabytes `volumes` lists where the staging could go: the local volumes the inventory declares on that member, then every ext4, xfs or btrfs file system mounted there from a local disk outside the system's own trees, each with its `device`, `size_bytes` and `free_bytes`. An RBD image mounted there is not a local disk and is left out. |
| GET | `/backup/connection` | Whether each cluster member can push to the backup server, every member asked at once over one SSH connection each: the public half of its backup key, and a connection to the server made the way a backup makes it, with `BatchMode`. `reaches` is `null` for a member that could not be asked. `key_path`, `uses_key` and `host_keys` say what the inventory holds: the dedicated key, whether `remote_shell` names it, and the server's confirmed host keys with their fingerprints. The connection runs `df -Pk` on `remote_dir` rather than `true`, so `space` carries the size and free space of the file system the backups land in, as the member the backups run on saw it, or `note` with what `df` said when it could not measure the directory. A directory that is not there yet is measured on its nearest existing parent, and `missing` counts the levels absent: the backups end in `rsync`, which creates one and no more. The page holds it, with the staging directory's free space, against the estimate of a full backup |
| POST | `/backup/connection/scan` | The backup server's host keys, read with `ssh-keyscan` from this node on the port `remote_shell` names, each with its fingerprint and the `known_hosts` line it would be committed as. Writes nothing. `admin` |
| PUT | `/backup/connection/key` | Commit `host_keys`, the confirmed lines, as `backup_restore_remote_host_keys`, with `backup_restore_ssh_key` naming a dedicated key and `remote_shell` gaining `-i` with it, as one commit on `cluster_machines`. A run of `seapath_setup_backup_restore` then generates the key on every member. `admin` |
| POST | `/backup/connection/install` | `ssh-copy-id` for every member at once: the members' public keys, read now, appended to `authorized_keys` of the account on the backup server, over one connection with `password`. Refused before the password is sent if the server answers with a host key other than the confirmed one. The password is neither stored nor logged. Answers the connection as read again afterwards. `admin`. See [D57](decisions.md#d57) |
...
one of them with `rm -rf "${local_dir}"*`, and the script has no check of its
own: a value without its trailing slash removes every sibling whose name starts
the same way, and a short one removes a great deal more than that. Both are
refused with the sentence that says why, and so is a path of fewer than two
...
because `restore_vm.sh` recreates the guest from the libvirt XML of the date it
is given and a date with no XML there produces a guest built from a file that
is not on the machine, minutes after the confirmation destroyed the running
one.

**The confirmation the scripts ask for is asked by the page.** Each of the
three acts pauses on a bare `read -r` before it deletes something. A task's
standard input is `/dev/null`, so that prompt would be answered by accident;
the generated plays answer it with `stdin` on purpose, and the window on the
page asks the question instead, naming what is about to be erased. A full
backup purges the snapshots of every image it exports, which retires the
increments of the previous backup, and that is in the confirmation.

**A backup holds the cluster's run lock for as long as it takes.** It is not
carved out of the one run at a time rule: a backup reads every image of the
...
| POST | `/software/update` | Run `seapath_update_debian` on `hosts`, one machine at a time, then the check on the same machines, as one generated run filed under `software_update`. `202` with the run. When `hosts` names the machine serving the API, the playbook is given `defer_reboot`, and the run schedules the reboot it decided after the check. Refused with `controller_not_alone` when `hosts` names the machine serving the API beside others, which are updated first, with `controller_in_scope` when it names that machine and the installed playbook still finishes on the controller, and with `one_machine_at_a_time` when the installed playbook has no `serial: 1` and `hosts` names several. `admin` |
...
The journal of every machine the inventory declares, asked over the SSH path a
run takes, merged here and stored nowhere. See [D63](decisions.md#d63).
...
than seeking through its index: measured on a 517 MB journal, 2.9 seconds of a
hypervisor's CPU against 9 ms for the same search behind a unit match, and
4.6 MB of JSON for an unmatched hour against 5 KB. A query with none is
refused with `409` and `precondition_failed` naming the condition, and `grep`
applies to what the matches already selected.

**A machine that does not answer is a row, never a failure.** It is named in
`machines` with the reason and in `warnings`, and the lines of the machines
that answered are returned beside it. The machines are asked in parallel, so a
reading costs the slowest of them: 165 ms for a three node cluster, measured.

**The window is absolute and in UTC.** `since` defaults to the last fifteen
minutes. A timestamp with no zone is read as UTC rather than as anything
local, and every bound reaches `journalctl` suffixed with `UTC`, because the
machines of one inventory need not share a timezone.

**Nothing is stored.** What a reading returns is what the machines still hold.
A site that needs the journal of a machine that is gone sends it to a
collector, which the upstream `syslog_ng_client` role is the supported way to
do.

## Usage

What every machine the inventory names consumes, and each guest and container
on it, over the last five minutes. See [D67](decisions.md#d67).

| Method | Path | Description |
|---|---|---|
| GET | `/usage` | The window this service kept, one row per machine with an address, in the inventory's order. `now` is this service's clock, which every `at` is on, with `period_seconds` and `window_seconds`. Each machine carries `points`, oldest first: `at`, `memory` (`total`, `used`, `reserved` for the hugepage pool), and against the reading before it `cpu` (`housekeeping` and `isolated` CPUs busy, with `housekeeping_cpus` and `isolated_cpus`), `disk_read`, `disk_write`, `network_in` and `network_out` in bytes per second over the physical disks and ports, and `workloads`, each running one's `cpu` and `memory` by `vm:<name>` or `container:<name>`. `latest` is the last reading as the exporters gave it: the machine's filesystems, disks and interfaces with their `kind`, its guests and containers, and whether each exporter answered in `node_reach`, `guests_reach` and `containers_reach`. `workloads`, `disks` and `interfaces` are the rates of that last reading, for the tables. `since` returns only the points taken after it. `viewer` |

**Read while somebody is here.** A recorder reads `node_exporter`,
`libvirt-exporter` and `prometheus-podman-exporter` on every machine every
`usage_period_seconds`, for as long as a request carrying a live session
arrived in the last `usage_idle_seconds`, and keeps `usage_window_seconds` of
points in memory. Asking this endpoint is such a request. The first request
after a quiet spell reads the machines itself, so its answer holds one point
and the rates start with the next.

**Rates of two readings.** A point after a gap in the readings, of more than
three periods, carries its memory and no rate. A counter lower than in the
reading before was reset, by a reboot or a guest started again, and gives no
rate for that interval. A figure the exporter did not publish is `null`.

**Around the scrape window.** The recorder's readings are never answered from
the few seconds of answers the other pages share, and do not empty it: a rate
divides by the time between two readings, and a kept answer carries the time
of another one. The endpoint takes no `fresh`.

**A machine is its three exporters, each on its own.** One that answers
nothing on `node_exporter` is a row with the reason and no rates. One outside
the `hypervisors` group runs no `libvirt-exporter` and no
`prometheus-podman-exporter`, which is its ordinary state and is said in
`guests_reach` and `containers_reach`.

**The machine's traffic is its physical ports.** A team, a bridge or a VLAN
carries the traffic of the ports under it again, and a guest's or a container's
end of its link carries that workload's, which the workload's own row reports.
A port is an interface whose address the hardware came with,
`node_network_address_assign_type` 0. A guest's tap device is a workload's end
whatever its name, since libvirt names it in the guest's own series.
