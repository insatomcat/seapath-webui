...
| PUT | `/realtime/pool/{host}/strategy` | Set one machine's `seapath_alloc_strategy` to `spreading`, `packing` or `repacking`, as one commit on its entry, `If-Match` on the commit hash. Then one run of `seapath_setup_deploy_seapath_alloc` narrowed to that machine templates `/etc/seapath/alloc.yaml`, which `seapath-alloc` reads at its next allocation. Answers with the commit and `run_id`, neither when the machine already receives this value, the role default `spreading` included. `404 unknown_host`, and `409 precondition_failed` before anything is committed when the playbook cannot run from here. Each node of `/realtime/pool` carries the value it receives as `alloc_strategy`, and `alloc_strategy_on` says where the inventory sets it: `host` or a group name. `admin` |
