...
| PUT | `/vms/{name}/pinning-profile` | Write a standalone guest's `vm_pinning_profile`, as one commit on its entry, `If-Match` on the commit hash; an empty `profile` takes it out. Answers with the commit, none when the entry already said this, `playbook`, the run that writes it to `/etc/seapath/alloc.d`, and `host`, the machine to narrow that run to. The playbook is `seapath_setup_deploy_seapath_alloc`, which touches no guest; with a collection whose `deploy_seapath_alloc` has no `tasks/profiles.yml` it is the guest's deployment, which writes it too and starts every enabled guest. `400 invalid_guest` for a profile that is not a YAML mapping, and for a cluster guest, whose profile is `_seapath_alloc` in its image's metadata. `admin` |
