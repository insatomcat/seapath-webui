...
| POST | `/vms/{name}/forget-creation` | Take out of a guest's own entry what only its creation reads: `vm_disk`, `vm_template`, `xml_path`, `additional_disk`, `disk_extract` and `cloud_init`. One commit (`vms: forget how <name> was created`) and no run; the files stay in the artefacts and the inventory folder. Answers with the commit and the variables `forgotten`. `409 creation_still_read` for a guest nothing reports, which the next deployment run creates, or one carrying `force`; `409 invalid_guest` when the entry carries none of them. See [D49](decisions.md#d49). `admin` |
