...
| `/vms` | The guests: what the inventory declares, whether a run would find the files each one names, and Pacemaker's line for it. Read only |
...
Open to the `viewer` role, and read only. The definition is changed by a commit
through `/inventory`, and the guests are deployed by a run of
`deploy_vms_cluster` or `deploy_vms_standalone` through `/runs`. Neither gets a
second door here.
