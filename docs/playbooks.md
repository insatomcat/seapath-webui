...
| Playbook | Targets | Preview | Reboots | Notes |
|---|---|---|---|---|
| `seapath_update_debian.yaml` | `all` | none | when a kernel changes | Snapshots the root volume and runs `apt-get dist-upgrade`, one machine at a time, and reboots a machine only when the upgrade installs a kernel or one is installed and not booted, with the GRUB boot counter armed first. The snapshot is taken first, sized to root or to what the volume group has free, and too little room stops the run before anything changes. A cluster member that reboots is then put in standby and Ceph's `noout` is set; a failure before the reboot undoes both, and the machine undoes them itself once its new system is up. Without a reboot the guests stay where they are, since one that cannot migrate live would be stopped and started elsewhere, and the playbook removes the snapshot itself. When that upgrade brings a new `pacemaker` or `corosync`, whose packages stop and restart the cluster stack, the cluster is put in `maintenance-mode` for its length, unless an operator already did, and taken out once the machine is back in a quorate partition and its guests were probed again. The machine driving the run only alone. With `defer_reboot` it stops short of the reboot and leaves `update_debian_reboot` for the caller. See [D59](decisions.md#d59), [D60](decisions.md#d60) and [D61](decisions.md#d61). |
