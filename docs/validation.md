...
| 25 | `sudo -n sh -c id` answers `uid=0(root)` in the console, `sudo -n true` refuses, and the panel had already said which of the two works | The consequence the site has to accept before enabling this, said out loud rather than discovered | Pending |
...
| 20 | Deleting the files of that guest, from the mark beside its name, removes the uploaded image from the artefacts and a brought XML with a commit, the guest keeps running, and after a reboot of its node it still starts | The guest's disk living in Ceph or the libvirt pool, independent of the file it was imported from | Pending |
...
| 5 | **Show the backups** answers in a second or two and shows what check 1 and check 4 wrote, with the two dates the guest can be restored to | The one POSIX shell command against a real backup server's shell, over two real SSH hops | Pending |
| 5b | With the backup server unreachable, or its host key never accepted on that member, the panel says what ssh answered within about ten seconds rather than waiting | `BatchMode` and `ConnectTimeout` on the second hop. The first version had neither and sat there with the cluster's run lock held, which is the defect this checks has gone | Pending |
| 6 | The estimate on the page is at or above what check 1 actually transferred, and within a reasonable margin of it | `rbd du` read here against what a qcow2 export weighs. It is a ceiling by construction, so a figure below what was transferred is a defect and a figure far above it on a snapshot heavy pool is the bound being loose | Pending |
...
| 14 | **Measure it** on a real pool answers in about the time `rbd du` of the selected images takes at the member's shell, and the figure is close to what check 1 transferred | `rbd ls` and `rbd du` per image over SSH on a real member. The first cluster this met timed out at ten seconds, which moved it behind a button, and then took a minute from the container's half CPU, which moved it to the member | Pending |
| 15 | On a pool whose guests were snapshotted by last night's backup, no selected guest is reported at `0 B`, and a guest whose image reads thirty gigabytes is reported near thirty rather than near nothing | The deltas `rbd du` answers, which only a pool carrying snapshots produces. The first pool this met reported three of three guests at a few hundred megabytes | Pending |
