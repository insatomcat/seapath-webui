...
class Settings(BaseSettings):
    ...
    # Where `deploy_metrics_proxy` serves all of the above, each on its own
    # path and over TLS, on the machines that carry
    # `deploy_metrics_proxy_enabled`. Those machines answer on nothing else:
    # the role moves their exporters to the loopback, which is the whole point
    # of deploying it.
    metrics_proxy_port: int = 9464
    # The CA every metrics proxy certificate is verified against, for a site
    # that issues them. Left unset, each machine's certificate is read once over
    # the SSH connection a run already makes and pinned under `state_dir`,
    # which is the model the SSH host keys themselves use.
    metrics_proxy_ca_file: Path | None = None
    ...
    @property
    def metrics_proxy_cert_dir(self) -> Path:
        """Where each machine's pinned metrics proxy certificate is kept."""
        return self.state_dir / "metrics-proxy-certs"
