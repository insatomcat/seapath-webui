...
def test_a_pinning_profile_is_a_commit_on_the_guests_entry(
    ...
    response = signed_in.put(
        "/api/v1/vms/ABBICT/seapath-alloc", json={"profile": PROFILE}
    )
    ...
    assert body["run_id"]
    assert _entries(signed_in)["ABBICT"]["seapath_alloc"] == PROFILE
    guests = {g["name"]: g for g in signed_in.get("/api/v1/vms").json()["guests"]}
    assert guests["ABBICT"]["seapath_alloc"] == PROFILE
...
def test_the_same_profile_again_is_no_commit(
    ...
    _standalone(signed_in, remote_runner)
    signed_in.put("/api/v1/vms/ABBICT/seapath-alloc", json={"profile": PROFILE})

    again = signed_in.put("/api/v1/vms/ABBICT/seapath-alloc", json={"profile": PROFILE})
...
def test_an_empty_profile_takes_the_variable_out(
    ...
    response = signed_in.put("/api/v1/vms/ABBICT/seapath-alloc", json={"profile": ""})

    assert response.status_code == 200, response.text
    assert response.json()["commit"]
    assert not (_entries(signed_in)["ABBICT"] or {}).get("seapath_alloc")
...
def test_a_cluster_guests_profile_is_its_metadata(signed_in: TestClient) -> None:
    ...
    response = signed_in.put(
        "/api/v1/vms/vm-guest1/seapath-alloc", json={"profile": PROFILE}
    )
...
def test_the_profile_run_writes_the_committed_value_and_nothing_else(
    ...
    wait_for(signed_in, body["run_id"])
    play = _written(settings, body["run_id"], "seapath_alloc")[0]
    ...
    assert copy["ansible.builtin.copy"]["content"] == (
        "{{ hostvars[seapath_webui_guest].seapath_alloc }}"
    )
    ...
    assert remove["when"] == (
        "hostvars[seapath_webui_guest].seapath_alloc is not defined"
    )
...
def test_without_a_restart_the_guest_is_left_running(
    ...
    wait_for(signed_in, body["run_id"])
    tasks = _written(settings, body["run_id"], "seapath_alloc")[0]["tasks"]
